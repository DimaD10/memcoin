window.addEventListener('load', function () {
    ...
    let currentValue = 0;
    const targetValue = burnedValue + presaleValue + liquidityValue + marketingValue + airdropValue;
  
    const animate = () => {
      if (currentValue >= targetValue) {
        clearInterval(animation); // Зупиняємо анімацію після досягнення кінцевого значення
        return;
      }
  
      currentValue += 0.1; // Збільшуємо значення кожен крок анімації
